...
for u=1:S*S
    ...
    reward_course(t) = r;
    reward_mean(t) = R/t;
    [V(s0,pId,gId),a1]=max(Q(s1,:,pId,gId));
    if (rand(1)<epsilon) a1=randi(A);end;
    if maxV < V(s1,pId,gId)
        maxV = V(s1,pId,gId);
    end;
    time_course_SARSA(t,1)= Q(s1,a1,pId,gId);
    time_course_SARSA(t,2)= eta*(r+gamma*Q(s1,a1,pId,gId));
    time_course_SARSA(t,3)= (1-eta)*Q(s0,a0,pId,gId);
    Q(s0,a0,pId,gId)=(1-eta)*Q(s0,a0,pId,gId)+eta*(r+gamma*Q(s1,a1,pId,gId));
    if(pId==5)
        stepNo = stepNo + 1;
    end    
